...
export function ClassSelector({ roles, form, onToggleRole }: Props) {
  return (
    <Card
      variant="white"
      padding="none"
      className="border-4 shadow-[12px_12px_0_0_#B8FF29] overflow-hidden"
    >
          ...
          {roles.map((role) => {
            ...
            return (
              ...
              >
                {role}
                {isPrimary && (
                  <span className="text-[10px] bg-neo-lime text-black px-2 py-0.5 rounded-sm">
                    PRIMARY
                  </span>
                )}
                {isSecondary && (
                  <span className="text-[10px] bg-neo-black text-white px-2 py-0.5 rounded-sm">
                    SEC
                  </span>
                )}
              </button>
